                ...
                <div className="flex flex-col items-center mb-8">
                  <div className="relative group">
                    ...
                    <label className="absolute inset-0 flex items-center justify-center bg-black/40 text-white rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
                      <span className="text-xs uppercase" style={LG}>Change Image</span>
                      <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
                    </label>
                  </div>
                  <div className="flex gap-2 mt-2">
                    <p className="text-white/40 text-[8px] uppercase" style={LG}>Touch image to upload (Max 1MB)</p>
                    {formData.avatar && (
                      <button onClick={() => setFormData({...formData, avatar: ""})} className="text-red-400 text-[8px] uppercase hover:underline" style={LG}>REMOVE</button>
                    )}
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] uppercase mb-1 block ml-2" style={{ ...LG, color: "rgba(165,243,252,0.7)" }}>Nickname (Username)</label>
                    <input type="text" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value.toUpperCase()})}
                      ...
                      style={{ ...LG, letterSpacing: "0.05em" }} placeholder="CHOOSE A COOL NAME" />
                  </div>
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1">
                      <label className="text-[10px] uppercase mb-1 block ml-2" style={{ ...LG, color: "rgba(165,243,252,0.7)" }}>First Name</label>
                    ...
                    <div className="flex-1">
                      <label className="text-[10px] uppercase mb-1 block ml-2" style={{ ...LG, color: "rgba(165,243,252,0.7)" }}>Last Name</label>
                      <input type="text" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})}
                        className="w-full bg-[#071428] text-cyan-100 border-4 border-cyan-500/60 rounded-xl p-3 outline-none" style={LG} />
                    </div>
                  </div>
                  <div className="flex gap-4 pt-4">
                    <button onClick={() => setEditMode(null)} className="flex-1 bg-[#0a2f5e] text-cyan-200 py-3 rounded-xl border-4 border-cyan-700 shadow-[0_4px_0_0_#041229] uppercase" style={{ ...LG, letterSpacing: "0.05em" }}>Cancel</button>
            ...
            {!editMode && (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                  {[
                    ["Full Name", `${user?.first_name || "NOT"} ${user?.last_name || "SET"}`],
                    ["Email Address", user?.email || "N/A"],
                    ["Current Level", `Level ${profile.current_level}`],
                    ["Total Score", `${profile.total_marks} Points`]
                  ].map(([label, value]) => (
                  ...
                  <div className="mb-10">
                    ...
                    <div className="rounded-2xl overflow-hidden border-4 border-cyan-500/40">
                      <div className="grid grid-cols-4 bg-[#0a2f5e] px-4 py-2">
                        {[["Level",""],["Time","text-center"],["Stars","text-center"],["Status","text-right"]].map(([h, align]) => (
                        ...
                        <div key={lvl.level_number} className={`grid grid-cols-4 px-4 py-3 items-center border-t-2 border-cyan-500/20 ${idx % 2 === 0 ? "bg-[#071428]" : "bg-[#0a1c3d]"}`}>
                          ...
                          <span className="text-right">
                            {lvl.stars_earned >= 2.5 && lvl.total_time_seconds > 0
                              ? <span className="text-[10px] uppercase px-2 py-1 bg-cyan-500/20 text-cyan-300 rounded-lg border border-cyan-500/40" style={{ ...LG, textShadow: "0 0 8px rgba(74,234,220,0.6)" }}>Perfect</span>
                              : lvl.stars_earned >= 1
                              ? <span className="text-[10px] uppercase px-2 py-1 bg-orange-500/20 text-orange-300 rounded-lg border border-orange-500/40" style={LG}>Done</span>
                              : <span className="text-[10px] uppercase px-2 py-1 bg-white/10 text-white/40 rounded-lg border border-white/20" style={LG}>Tried</span>
                            }
                          </span>
                        </div>
                      ))}
